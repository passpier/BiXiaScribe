...
- [ ] 1.1 Add `FIELD_HELP: dict[str, dict[str, str]]` (keys matching `_CUSTOM_FIELDS`) with
...
- [ ] 1.2 Add `_AFFECTS_LABEL` mapping `{"legacy": ..., "layered": ..., "both": ...}` for the UI
      warning suffix.

## 2. `src/bixiascribe/catalog.py` + `eval/model_catalog.json` — model catalog foundation

- [ ] 2.1 Create `eval/model_catalog.json`: `models` (deepseek-v4-flash-0731 tested,
...
- [ ] 2.2 `catalog.py`: `ModelInfo`/`RoleInfo`/`Catalog` frozen dataclasses; `load_catalog(path,
...
- [ ] 3.1 Add `role_keys_for_mode(mode: str) -> tuple[str, ...]` and `run_role_models(run:
      RunRecord) -> list[tuple[str, str]]`, with the layered set including `"proof"` (see design.md
      實測四).
- [ ] 3.2 Split `_read_envelope()` into `_read_envelope_versioned(path) -> tuple[int | None, dict |
...
- [ ] 3.3 Add `ResumableRun` frozen dataclass (`run_id, path, requirement, stage,
...
- [ ] 3.4 Add `RunRecord.reasoning_effort: str = ""` field, read via `row.get("reasoning_effort")
      or ""`.
- [ ] 3.5 Update `ScriptRecord`'s docstring/comment (currently missing `"checkpoint"`) to enumerate
      all four `source` values including the new `"adhoc"`.
- [ ] 3.6 Add `reasoning_effort` to `overview_rows()`'s output dict.

## 4. `src/bixiascribe/library.py` — new module: delete/export/import

- [ ] 4.1 `DeletePlan` frozen dataclass; `plan_delete(rec, *, state_dir=...) -> DeletePlan`;
      ...
      `state_dir`.
- [ ] 4.2 `export_bytes(script) -> bytes` (byte-identical to `generation.py:598`'s
      `model_dump_json(indent=2, exclude_none=False)`), `export_filename(rec) -> str`.
- [ ] 4.3 `ImportRejected(ValueError)`; `validate_import(payload: bytes) -> Script` (parses JSON,
...
- [ ] 4.4 `import_script(payload, *, variant, requirement, rep=None, scripts_dir=...) -> Path` —
      writes via `generation.script_path_for()` + `generation.next_rep()`.
- [ ] 4.5 `load_ad_hoc(path: Path) -> tuple[Script, ScriptRecord]` — returns `source="adhoc"`,
      `run=None`.

## 5. `src/bixiascribe/generation.py` — dedup role list, reasoning effort, resume plumbing

- [ ] 5.1 Rewrite `_cost_models()` (`:326-340`) to build its role dict from
      `review.role_keys_for_mode(report.mode)` instead of a hardcoded literal, for both legacy and
      layered branches.
- [ ] 5.2 `Variant.reasoning_effort: str | None = None` (appended last, per the existing
...
- [ ] 5.3 `generate(..., reasoning_effort: str | None = None)` — three-level resolution (explicit >
...
- [ ] 5.4 `GenerationJob.__init__(..., run_id: str | None = None, reasoning_effort: str | None =
...
- [ ] 5.5 Re-export `CHECKPOINT_SCHEMA_VERSION = crew.orchestrator._SCHEMA_VERSION` (already
...
- [ ] 6.1 `ModelChoice.reasoning_effort: str = config.REASONING_EFFORT`.
- [ ] 6.2 `build_llm()`: after normalizing via `catalog.normalize_reasoning_effort(models.reasoning_effort)`,
...
- [ ] 7.1 Add `REASONING_EFFORT`, imitating `CAUSAL_VALIDATION`'s validate-and-fallback pattern
...
- [ ] 8.1 `RunReport.reasoning_effort: str = ""` (next to `script_length`, `pipeline.py:161`).
- [ ] 8.2 Set it at both `RunReport` construction sites — `pipeline.py:438` (legacy) and
      `orchestrator.py:1340` (layered) — from `models.reasoning_effort`.

## 9. `ui/app.py` — wire everything into widgets

- [ ] 9.1 Module-level `_CATALOG = catalog.load_catalog()` alongside existing cached data access.
- [ ] 9.2 `_render_run_meta()`: replace the two hardcoded 3-column model blocks with one loop over
...
- [ ] 9.3 `_render_script()`: add `st.download_button` for `library.export_bytes(script)` after the
      metrics row.
- [ ] 9.4 Add `@st.dialog("確認刪除")` confirm flow wired into the 單篇閱讀 branch and the
      unreadable-file branch (`:789-790`); clear `st.cache_data` **before** `st.rerun()`; add a
      bulk "刪除所有無法讀取的劇本" action.
- [ ] 9.5 Add sidebar import UI: `st.file_uploader` + variant/requirement inputs + explicit import
...
- [ ] 9.6 生成 form: reorder so `use_layered` checkbox precedes the variant/自訂 block (自訂模型's
...
- [ ] 9.7 生成 form: add a "續跑未完成的執行" expander from `review.discover_resumable_runs()`; on
      ...
      checkpoint.
- [ ] 9.8 `gen_last_result` branch: add the same export button as 9.3.
- [ ] 9.9 自訂篇幅 block (`:654-671`): loop over `length.FIELD_HELP`, add `help=` text, append
...
- [ ] 10.1 `tests/test_catalog.py` (new): bidirectional id-set consistency between
...
- [ ] 10.2 `tests/test_library.py` (new), tmp-dir only: `export_bytes` byte-identical to
...
- [ ] 10.3 `tests/test_review.py` (modified): `discover_resumable_runs` cases mirroring
      ...
      the versioned split.
- [ ] 10.4 `tests/test_generation.py` (modified): `GenerationJob(run_id=...)` actually resumes an
      ...
      `reasoning_effort`.
- [ ] 10.5 `tests/test_llm_reasoning.py` (new): mock `crewai.LLM`, assert `reasoning_effort="default"`
...
- [ ] 10.6 `tests/test_length_spec.py` (modified): `set(FIELD_HELP) == set(_CUSTOM_FIELDS)`; every
      `affects` value is one of `{"legacy","layered","both"}`.
- [ ] 10.7 `tests/test_pricing.py`: deliberately unmodified (see design.md 決策六).
- [ ] 10.8 Run `pytest tests/` and `ruff check .` clean.

## 11. Docs

- [ ] 11.1 `CLAUDE.md`: update the Review UI description, add a `REASONING_EFFORT` line to the env
      var documentation.
- [ ] 11.2 `.env.example`: add `REASONING_EFFORT` (commented, default `default`).
- [ ] 11.3 `README.md`/`README.en.md`: update the "📋 規劃中" line — import/export/delete land;
      editing-in-place and RPG Maker export remain future work.
- [ ] 11.4 `CONTRIBUTING.md` (or wherever the streamlit-free rule is documented, if it exists as a
...
- [ ] 12.1 Offline: `ruff check .`; `pytest tests/`.
- [ ] 12.2 `LLM_BACKEND=fake .venv/bin/streamlit run ui/app.py` manual walkthrough: delete a broken
