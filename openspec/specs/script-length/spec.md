...
a fixed set of five prompt targets: event count, chapter count, beats per chapter, minimum
dialogue depth, and main/flavor scene mix ratio.
...
- **THEN** the system resolves that preset's five targets, including a scene-mix target
  expressing that main scenes should be somewhat more numerous than flavor scenes
...
- **WHEN** a custom value supplies all five fields (`events`, `chapters`,
...
- **THEN** the system derives the missing fields from `events` (chapters and beats per
  chapter scaled proportionally to `events`, minimum dialogue depth and scene mix tiered by
  `events`) and the resolved specification includes concrete values for all five fields
...
the fully-resolved script-length specification — with all five fields filled in — rather than
