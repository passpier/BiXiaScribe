...
_ORIG_WRITER = '根據以下使用者劇情需求，設計一份武俠 RPG 事件/分支骨架：\n\nREQ\n\n產出必須包含：title、premise、theme（主題一句話）、goal（玩家目標）、tone（基調/氛圍）、至少 1-2 個 variables、至少 2 個 npcs（含 id/name/identity/personality/speech_style，以及可選的 faction_id/surface_motive/true_motive）、至少 2 個 events。每個 event 需含 id/title/location/summary、triggers、branches（branch.next_event_id 必須對應到某個 event 的 id），並且每個 event 的 dialogue 欄位一律填空陣列 []——台詞由下一位「對話 agent」負責，你只搭骨架。至少 1 個 chapter（含 hook、converge_event_id 指向章節內分支最終收斂的 event）。另外，這是一齣要「玩」的 RPG，不是純小說大綱，所以還必須包含：player（玩家角色，id/name/identity，stats 至少 2 個數值型屬性，如內力/聲望/銀兩，kind 設為 "stat"）——玩家絕對不可以放進 npcs 名冊，旁白也不要造一個假 NPC，敘述請寫進 event.summary；items（至少 1-2 件關鍵道具，每件都要有 acquired_in_event_id 指出在哪個 event 可以取得，或留白表示一開始就持有）；quests（至少 1 條任務，objective 一句話說清楚目標，event_ids 指向推進此任務的實際 event id）。每個 npc 都要填 first_appearance_event_id（此角色第一次登場的 event id）與 introduction（如何被引見），NPC 不能在自己first_appearance_event_id 之前的場次講話。還要設計這個江湖的世界觀骨架：factions（勢力，至少 1-2 個，含 id/name/alignment，以及 relations 列出與其他勢力的關係，stance 用「結盟／敵對／中立／附庸」描述）；stat_thresholds（陣營值/數值門檻表，每個會被 branch 效果改動的 stat 都至少要有一條門檻規則，寫清楚 min_value/max_value 這個區間解鎖了什麼——unlocks_kind 填 branch/event/npc_attitude/ending 之一，unlocks_id 填對應的 id，區間彼此不可重疊）；regions（至少 1 個地區，含 unlock_condition，以及至少 2 個 sub_locations，每個都要有 function 說明用途，如打聽消息/交易/療傷/學習技能）；truth（三層真相：public 是一開始就公開的事實，progressive 是隨章節逐步揭露的事實——每條都要填 reveal_chapter_id 指出在哪一章揭露，hidden 是保留給作者、絕對不能提前出現在任何場景內容裡的私藏真相）；endings（至少 1-2 個結局，每個都用 stat_conditions 和/或 required_branch_ids 說明怎麼達成）。每個有結構化效果（effect_ops）的分支選項都要符合「抉擇點設計三原則」：1) 代價（cost）——玩家真正失去了什麼，不能只是數值增減，要讓玩家覺得「這是取捨」；2) 立即回饋（immediate_feedback）——選了之後馬上看得到後果；3) 延遲回收（payoff_chapter_id/payoff_description）——如果效果不是當場兌現，要說明在哪一章、如何兌現，最終所有分支都要能收斂（converges_to_event_id）回主線——converges_to_event_id 是額外欄位，next_event_id 仍然必填，不可省略。\n錯誤示範（假選擇）：「A. 立刻上前扶起受傷的少女」與「B. 立刻上前扶起受傷的少年」——兩者文字幾乎相同、效果目標相同（都是聲望+1），沒有任何取捨可言，這是假選擇，不允許。\n正確示範：「A. 出手救人（代價：暴露行蹤，日後被追殺，聲望+1，內力-10）」與「B. 悄悄離開（代價：錯過一條重要線索，但保住行蹤）」——兩者代價不同、指向不同的後續發展，這才是真選擇。'  # noqa: E501
_ORIG_DIALOGUE = (
...
_ORIG_BEAT_EXPAND = '使用者的劇情需求：\n\nREQ\n\n拆書人已整理出的人物/變數素材：\n\n{"npcs":[],"variables":[],"player":null,"items":[],"quests":[],"props":[],"branch_candidates":[],"theme":"","goal":"","tone":"","factions":[],"regions":[],"truth":null,"stat_thresholds":[],"clues":[],"endings":[]}\n\n請把這段劇情排成分章大綱與逐場戲的 beat 清單：至少 1-2 章（chapters），每章至少 1 個 beat。每個 chapter 需含 hook（開場鉤子，一句話勾住玩家）、converge_event_id（這一章分支最終收斂回主線的 event id——此時 event 還沒寫出來，先填一個之後場景會用到的 id 佔位，例如 "ev-ch1-converge"）。每個 beat 需含 id、chapter_id（對應到某個 chapter 的 id）、summary（這場戲的梗概）、scene_kind（"main" 表示推進真相的主要場景，"flavor" 表示調味場景，主要場景略多於調味場景）、npc_ids（涉及哪些登場角色，id 需來自上方素材）、causal_deps（依賴哪些前置 beat 的 id，沒有就留空陣列）。不要寫場景細節或台詞，那是後面「江湖代言人」的活兒。'  # noqa: E501
_ORIG_SCENE_WRITE = '請把以下這一場戲的 beat 展開成一個完整的 event。session 內含玩家/道具/任務素材、登場 NPC 設定（含哪些已經在先前場次登場過）、勢力/門檻表/章節/地區、目前已解鎖的真相（truth_public/truth_unlocked——這是本場戲能提及的真相全部，絕對不能提及尚未解鎖的內容）、目前這場戲的 beat，以及（若有）已完成的前情場次摘要——已完成場次是本場戲不可牴觸的既定事實：\n\n{"character_cards":[],"scene_summaries":[],"omitted_scene_count":0,"player_card":[],"item_cards":[],"quest_cards":[],"introduced_npc_ids":[],"faction_cards":[],"threshold_card":[],"chapter_card":[],"region_card":[],"truth_public":[],"truth_unlocked":[],"allowed_ids":[],"current_beat":{"id":"b1","chapter_id":"c1","summary":"s","npc_ids":[],"causal_deps":[],"scene_kind":""}}\n\nevent 的 id 欄位請填 "e1"。依每位 NPC 的 identity/personality/speech_style，使用語料庫檢索工具（wuxia_corpus_search）查詢貼近場景語感的原文片段，寫出至少一段台詞。location、triggers、branches 依 beat 的 summary、已完成場次摘要與因果合理補上，不得與已完成場次矛盾。scene_kind 請填 "main"（推進真相）或 "flavor"（調味），chapter_id/region_id/sub_location_id/clue_ids/checks 裡的 item_bypass_id，以及 branch 的 payoff_chapter_id/converges_to_event_id，只能填 session.allowed_ids 這份清單裡列出的值——這是封閉選單，不是自由發揮，清單裡沒有合適的就留空，絕對不要自己編一個新 id；若本場戲有需要檢定的橋段，填 checks（每個 SkillCheck 都要有 failure_branch_id 或 item_bypass_id 其中之一，且有 failure_cost，確保失敗也能推進劇情）；若本場戲有可蒐集的線索，填 clue_ids。若本場戲有 NPC 是第一次登場（不在已登場名單內），台詞或 summary 要交代清楚他是誰、為何在此，不可以憑空開口；branches 的效果請同時寫進 effect_ops（結構化：target_kind/target_id/op/value），effects 欄位留一句話人可讀摘要即可。每個有結構化效果（effect_ops）的分支選項都要符合「抉擇點設計三原則」：1) 代價（cost）——玩家真正失去了什麼，不能只是數值增減，要讓玩家覺得「這是取捨」；2) 立即回饋（immediate_feedback）——選了之後馬上看得到後果；3) 延遲回收（payoff_chapter_id/payoff_description）——如果效果不是當場兌現，要說明在哪一章、如何兌現，最終所有分支都要能收斂（converges_to_event_id）回主線——converges_to_event_id 是額外欄位，next_event_id 仍然必填，不可省略。\n錯誤示範（假選擇）：「A. 立刻上前扶起受傷的少女」與「B. 立刻上前扶起受傷的少年」——兩者文字幾乎相同、效果目標相同（都是聲望+1），沒有任何取捨可言，這是假選擇，不允許。\n正確示範：「A. 出手救人（代價：暴露行蹤，日後被追殺，聲望+1，內力-10）」與「B. 悄悄離開（代價：錯過一條重要線索，但保住行蹤）」——兩者代價不同、指向不同的後續發展，這才是真選擇。'  # noqa: E501
