...
_ORIG_SCENE_WRITE = '請把以下這一場戲的 beat 展開成一個完整的 event。session 內含玩家/道具/任務素材、登場 NPC 設定（含哪些已經在先前場次登場過）、勢力/門檻表/章節/地區、目前已解鎖的真相（truth_public/truth_unlocked——這是本場戲能提及的真相全部，絕對不能提及尚未解鎖的內容）、目前這場戲的 beat，以及（若有）已完成的前情場次摘要——已完成場次是本場戲不可牴觸的既定事實：\n\n{"character_cards":[],"scene_summaries":[],"omitted_scene_count":0,"player_card":[],"item_cards":[],"quest_cards":[],"introduced_npc_ids":[],"faction_cards":[],"threshold_card":[],"chapter_card":[],"region_card":[],"truth_public":[],"truth_unlocked":[],"current_beat":{"id":"b1","chapter_id":"c1","summary":"s","npc_ids":[],"causal_deps":[],"scene_kind":""}}\n\nevent 的 id 欄位請填 "e1"。依每位 NPC 的 identity/personality/speech_style，使用語料庫檢索工具（wuxia_corpus_search）查詢貼近場景語感的原文片段，寫出至少一段台詞。location、triggers、branches 依 beat 的 summary、已完成場次摘要與因果合理補上，不得與已完成場次矛盾。scene_kind 請填 "main"（推進真相）或 "flavor"（調味），chapter_id/region_id/sub_location_id 請對應到 session 裡的章節/地區設定；若本場戲有需要檢定的橋段，填 checks（每個 SkillCheck 都要有 failure_branch_id 或 item_bypass_id 其中之一，且有 failure_cost，確保失敗也能推進劇情）；若本場戲有可蒐集的線索，填 clue_ids。若本場戲有 NPC 是第一次登場（不在已登場名單內），台詞或 summary 要交代清楚他是誰、為何在此，不可以憑空開口；branches 的效果請同時寫進 effect_ops（結構化：target_kind/target_id/op/value），effects 欄位留一句話人可讀摘要即可。每個有結構化效果（effect_ops）的分支選項都要符合「抉擇點設計三原則」：1) 代價（cost）——玩家真正失去了什麼，不能只是數值增減，要讓玩家覺得「這是取捨」；2) 立即回饋（immediate_feedback）——選了之後馬上看得到後果；3) 延遲回收（payoff_chapter_id/payoff_description）——如果效果不是當場兌現，要說明在哪一章、如何兌現，最終所有分支都要能收斂（converges_to_event_id）回主線——converges_to_event_id 是額外欄位，next_event_id 仍然必填，不可省略。\n錯誤示範（假選擇）：「A. 立刻上前扶起受傷的少女」與「B. 立刻上前扶起受傷的少年」——兩者文字幾乎相同、效果目標相同（都是聲望+1），沒有任何取捨可言，這是假選擇，不允許。\n正確示範：「A. 出手救人（代價：暴露行蹤，日後被追殺，聲望+1，內力-10）」與「B. 悄悄離開（代價：錯過一條重要線索，但保住行蹤）」——兩者代價不同、指向不同的後續發展，這才是真選擇。'  # noqa: E501
