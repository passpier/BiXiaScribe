...
- ✅ Stage 3: Streamlit UI (`streamlit run ui/app.py`) — three read-only modes for browsing and
  side-by-side comparing scripts generated by different model splits, plus a 生成 (generate) mode
  that triggers a real run from the browser (background job, live progress, cancellable).
