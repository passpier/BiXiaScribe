...
# 5. Read-only review / side-by-side comparison of already-generated scripts
#    (no API key, no tokens spent)
...
- ✅ Stage 3 (first slice): **read-only** Streamlit review UI (`streamlit run ui/app.py`) for browsing
  and side-by-side comparing scripts generated by different model splits.
- 📋 Streamlit generation UI (triggering generation from the UI) / RPG Maker export (planned)
