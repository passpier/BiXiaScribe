...
- A Streamlit UI (`ui/app.py`) with three read-only modes for browsing/comparing scripts already
  ...
  directly from the browser. The review UI can also delete a script record's file (or, for a
  checkpoint-sourced record, its whole `.bixia_state/<run_id>/` directory), export a loaded script
  as JSON, and import a script (upload into `out/eval/`, or load an arbitrary path ad hoc for
  one-off viewing) — see `src/bixiascribe/library.py`. Model selection in the 生成 form is
  constrained to a curated catalog (`eval/model_catalog.json` + `src/bixiascribe/catalog.py`), and
  an interrupted layered run can be resumed from the form (hard-gated on checkpoint schema version
  — see `src/bixiascribe/review.py::discover_resumable_runs()`).

Editing/save-back from the UI (editing a script's *content* in place) and any RPG Maker
export/conversion are still not implemented — do not assume they exist yet.
