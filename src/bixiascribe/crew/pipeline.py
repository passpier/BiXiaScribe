...
def _repair(script: Script, problems: list[str], agent: Any) -> tuple[Script | None, str | None]:
    ...
    task = Task(
        description=(
            ...
            + "\n修正原則：每個 dialogue 的 npc 必須指向已存在的 NPC 或 "
            "\"player\"；每個 choice 的 next 必須指向已存在的 event；"
            "choice 的 payoff_at、npc 的 faction_id、event 的 "
            "chapter_id/clue_ids/npc_ids、check 的 on_pass/on_fail、"
            "chapter 的 start_event、clue/item 的 from_event，"
            "都必須指向已存在的對應項目。"
            "不要新增或刪除事件、NPC 或選項，只修正指向錯誤的欄位。"
        ),
        expected_output="修正後、交叉引用完全正確的完整 Script JSON。",
        agent=agent,
        # Phase 2: use the lenient mirror here too, not the strict Script --
        # this was the one remaining output_pydantic still exposed to the
        # ValidationError-inside-execute_sync() failure mode wire.py exists
        # to fix everywhere else (see wire.py's module docstring).
