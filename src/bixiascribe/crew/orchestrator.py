...
from . import causal, execute, guardrails, normalize, scene_metrics
from .agents import (
...
def _scene_meta_path(run_id: str, beat_id: str) -> Path:
    """Per-scene execution attribution sidecar (crew/scene_metrics.py::
    SceneMetric), written alongside _scene_path()'s Event checkpoint. Kept
    as a distinct file, same rationale as _pending_scene_path() -- so
    detect_stage() (which only ever consults _scene_path()) never needs to
    change, and a run resumed from before this sidecar existed simply has no
    sidecar for its already-committed scenes rather than failing to load.
    Not a stage marker: like _graph_path()'s causal graph, it's a derived
    artifact of the scene checkpoints, not consulted by detect_stage()."""
    return state_dir(run_id) / f"scene_meta_{beat_id}.json"


def load_scene_metrics(run_id: str) -> list[dict[str, Any]]:
    """Every committed scene's SceneMetric, in beat_sheet order, as
    JSON-safe rows -- what run_layered()'s _finalize_report() puts on
    RunReport.scene_metrics. Reads sidecars back from disk rather than
    relying on the in-process scene_metrics.get_stats() accumulator alone,
    since a resumed run's already-completed scenes were generated by an
    earlier process and never touch this process's in-memory accumulator at
    all.

    Filtered to beats whose _scene_path() Event checkpoint actually exists
    -- a pending/rejected batch's sidecar (if any were ever written for a
    staged scene) must never leak into a finished run's report, mirroring
    _refresh_graph()'s "only ever built from committed scenes" rule. A
    sidecar that's missing or fails to load (e.g. a scene generated before
    this capability existed) is simply skipped, not an error -- same
    "missing means not measured, not broken" convention as every other
    load_checkpoint() call in this module."""
    ...
    for beat in beat_sheet.beats:
        if load_checkpoint(_scene_path(run_id, beat.id), Event) is None:
            continue
        metric = load_checkpoint(_scene_meta_path(run_id, beat.id), scene_metrics.SceneMetric)
...
def _default_write_scene(
    ...
    *,
    ...
    with scene_metrics.scene_scope(beat.id):
        agent = make_scene_writer_agent(
            verbose=verbose, models=models, use_retrieval=use_retrieval
        )
        before = _llm_usage(agent)
        outcome = execute.run_task(
        ...
        output = outcome.output
        usage = _usage_delta(before, _llm_usage(agent))
        scene_metrics.record_usage(beat.id, usage)
...
def _default_repair_scene(
    ...
    scene_metrics.record_repair_elapsed(event.id, time.monotonic() - repair_start)
    usage = _usage_delta(before, _llm_usage(agent))
    scene_metrics.record_usage(event.id, usage)
    ...
    *,
    ...
    if stage == "extract":
        ...
        if target_beat is not None:
            ...
            metric = scene_metrics.get_stats().scenes.get(target_beat.id)
            if metric is not None:
                save_checkpoint(_scene_meta_path(run_id, target_beat.id), metric)
    ...
    if target_batch:
        ...
        with ThreadPoolExecutor(max_workers=min(concurrency, len(target_batch))) as pool:
            ...
            for future in as_completed(future_to_beat):
                ...
                metric = scene_metrics.get_stats().scenes.get(beat.id)
                if stage_pending:
                ...
                if metric is not None:
                    save_checkpoint(_scene_meta_path(run_id, beat.id), metric)
...
def run_layered(
    ...
    *,
    ...
    `on_report`, if given, is called once with `report` right after every
    `_finalize_report()` call (i.e. on every one of this function's exit
    paths, success or failure) -- this is what lets a caller like
    generation.generate() recover a *partial* RunReport (and therefore
    price whatever tokens were actually spent) even when this call ends by
    raising an exception `run_layered()` itself never converts to
    PipelineError, e.g. a `gate` callback raising GenerationCancelled to
    unwind a batch-confirmation wait -- see the `gate()` call site's own
    comment below for why that one exit needed an explicit
    `_finalize_report()` call of its own.
    """
    ...
    while True:
        ...
        if stage == "scenes" and gate is not None:
            ...
            # by _dispatch_batch_guarded()'s try/except -- it runs on the
            # caller's own thread and blocks waiting on a human decision
            # (see this function's docstring), so an exception here (e.g.
            # generation.GenerationCancelled unwinding a UI 取消 click) would
            # otherwise skip _finalize_report()/on_report() entirely and the
            # tokens already spent up to this point would go unaccounted
            # for. Re-raise unchanged -- unlike _dispatch_batch_guarded(),
            # this must not convert the exception's type, since callers
            # (e.g. GenerationJob._run()) branch on it.
            try:
                decision = gate(pending_ids)
